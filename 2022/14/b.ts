...
const data = input.split("\n").map((row) => {
  return row
    .split(" -> ")
    .map((coord) => coord.split(",").map((point) => +point));
});
...
const grid: string[][] = [];
for (let i = 0; i <= y.max + 2; i++) {
  grid.push([]);
  for (let j = 0; j <= compensate * 2 + x.max - x.min; j++) {
...
for (let i = 0; i < data.length; i++) {
  for (let j = 1; j < data[i].length; j++) {
    ...
    if (prev[0] === next[0]) {
      ...
      let drawing = true;
      do {
        grid[from][prev[0] - x.min + compensate] = "#";
    ...
    if (prev[1] === next[1]) {
      let from = prev[0] - x.min + compensate;
      const to = next[0] - x.min + compensate;
...
function fall([x, y]: [number, number]) {
  if (y === 0 && grid[y + 1][x - 1] === "o" && grid[y + 1][x + 1] === "o") {
    full = true;
  }
  const block = ["#", "o"];
